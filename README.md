...
| Opportunities          | The land bank (the two analysed plots plus real vacant parcels) ranked by the value a change of zoning category would add. Each plot's uses are priced under its current and alternative categories with the Financial Feasibility model; each area's demand gap is shared out plot by plot. Every plot gets a clear suggestion (recategorise X → Y for use Z, or keep), with NPV and revenue uplift, IRR, conditions and every option priced. |
